...
async function run() {
    ...
    try {
        ...
        try {
            ...
            const chunkUrl = `https://${OLD_DOMAIN}/mosaic_3d_zarr/0/9/0/0/0/0/0`;
        ...
        try {
            ...
            const newChunkUrl = `https://${NEW_DOMAIN}/mosaic_3d_zarr/0/9/0/0/0/0/0`;
        ...
        try {
            const corsResult = await new Promise((resolve) => {
                ...
                cp.exec(
                    `curl -s -H "Origin: https://evostitch.net" -D - -o /dev/null "https://${NEW_DOMAIN}/mosaic_3d_zarr/0/0/.zarray" 2>&1 | head -20`,
                    { timeout: 10000 },
